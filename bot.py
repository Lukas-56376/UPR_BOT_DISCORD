...
COLOR_END    = 0x99AAB5   # Gray   — win or lose
...
class PresidentView(discord.ui.View):
    ...
    def build_embed(self) -> discord.Embed:
        ...
        embed = discord.Embed(title="UPR - President Log", color=COLOR_ACTIVE)
        embed.set_author(
            name=self.author.display_name,
            icon_url=self.author.display_avatar.url,
        )
        embed.add_field(name="**__Lives__**",               value=hearts,                                              inline=False)
        embed.add_field(name="**__POTUS__**",               value=f"`{self.potus}`",                                   inline=True)
        embed.add_field(name="**__VPOTUS__**",              value=f"`{self.vpotus}`",                                  inline=True)
        embed.add_field(name="**Location Copy-Paste Command**",      value=f"```\n:h President: {self.potus} - Location: \n```", inline=False)
        return embed

    def build_end_embed(self, title: str) -> discord.Embed:
        embed = discord.Embed(title=title, color=COLOR_END)
        embed.set_author(
            name=self.author.display_name,
            icon_url=self.author.display_avatar.url,
        )
...
@app_commands.describe(
    potus="Roblox username of the POTUS",
    vpotus="Roblox username of the VPOTUS",
)
